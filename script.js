...
const API_KEY = "98ede5bae46f454bad4152114241910"; // Replace this with your actual API key
...
const getWeatherDetails = async (API_URL) => {
  ...
  try {
    ...
    const cityName = data.location.name; // Get the city name
    ...
    // Add the city name dynamically below the weather description
    if (!currentWeatherDiv.querySelector('.city-name')) {
      const cityElement = document.createElement('h4');
      cityElement.classList.add('city-name');
      currentWeatherDiv.appendChild(cityElement);
    }
    currentWeatherDiv.querySelector('.city-name').innerText = cityName;
...
// Initial weather request for Patna as the default city
setupWeatherRequest("Patna");
